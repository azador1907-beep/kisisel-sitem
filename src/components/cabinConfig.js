...
export function changeWall(
  ...
  if (linked && pair[wall]) {
    walls[pair[wall]] = apply(pair[wall]);
  }
...
export function mirrorGeometry(config) {
  const height =
    config.mirrorMode === 'none'
      ? 0
      : config.mirrorMode === 'full'
        ? 238
        : config.mirrorMode === 'half'
          ? 119
          : Math.max(
              60,
              Math.min(
  ...
  // Short mirrors stay at eye level,
  // with the three original panels beneath.
  const top = 119;
